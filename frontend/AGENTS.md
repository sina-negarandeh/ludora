...
No test framework is installed (no Vitest, Jest, React Testing Library, or Playwright). `npm run build` is the closest thing to an automated check on this side, necessary but not sufficient. If you add a framework, use Vitest + React Testing Library; it matches the existing Vite toolchain.

## Code style

`tsconfig.app.json` has `noUnusedLocals`, `noUnusedParameters`, `noFallthroughCasesInSwitch`, and `erasableSyntaxOnly` on. Do not disable any of them to make a build pass; fix the underlying issue instead. `oxlint` enforces `react/rules-of-hooks`.
...
`VITE_API_URL` (default `http://localhost:8000`) is the only env var this side reads. There's no dev-server proxy.
