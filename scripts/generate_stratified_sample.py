...
from absa_extract_hf import compute_quality_score, download_fasttext_model
import fasttext
...
def main():
    ...
    parser.add_argument('--top_n', type=int, default=ABSAConfig.SAMPLE_TOP_N_GAMES, help='Number of top ranked games to sample')
    parser.add_argument('--target_per_game', type=int, default=ABSAConfig.SAMPLE_TARGET_PER_GAME, help='Max reviews to sample per game')
    ...
    with engine.connect() as conn:
        ...
        for g_idx, game_id in enumerate(game_ids):
            ...
            for u_key, r_data in user_best_review.items():
                ...
                if rating is None or rating < ABSAConfig.SAMPLE_NEGATIVE_RATING_MAX:
                    b = 'neg'
                elif rating >= ABSAConfig.SAMPLE_POSITIVE_RATING_MIN:
                    b = 'pos'
                else:
                    b = 'mix'
    ...
    mlflow.log_params({
        "top_n": args.top_n,
        "target_per_game": args.target_per_game,
        "quality_score_threshold": ABSAConfig.QUALITY_SCORE_THRESHOLD,
        "negative_rating_max": ABSAConfig.SAMPLE_NEGATIVE_RATING_MAX,
        "positive_rating_min": ABSAConfig.SAMPLE_POSITIVE_RATING_MIN,
    })
    mlflow.log_metrics({"games_processed": len(game_ids), "reviews_sampled": total_sampled})

if __name__ == "__main__":
    with tracked_run("reviews/absa", run_name="stratified_sample"):
        main()
