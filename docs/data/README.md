...
| **Subdomain** | BGG's rank/leaderboard classification — which BGG sub-ranking a game appears on. Distinct from Category, though some categories (e.g. Wargame) automatically promote a game into the matching subdomain. | Strategy, Thematic, Family, Party, Wargame, Abstract Strategy, Children's, Customizable | Derived from rank data, not a `link` field |
| **Family** | The full `boardgamefamily` field: a large, flat bucket of user-curated groupings spanning 72 unrelated namespaces (`Animals`, `Mechanism`, `Crowdfunding`, `Country`, `Theme`, ...) | `Game: Catan`, `Crowdfunding: Kickstarter`, `Theme: Cthulhu Mythos`, `Series: ...` | `boardgamefamily` |
...
|---|---|---|
| Category | Dataset 2 `boardgamecategory` | Dataset 1 `themes.csv` (non-`Theme_` columns) + `subcategories.csv` — both verified to be the same BGG Category taxonomy under a different file name |
| Subdomain | Dataset 1 `Cat:*` flags OR Dataset 2 rank-column presence | — (both sources checked directly, no fallback needed) |
...
| Family | Dataset 2 `boardgamefamily`, the full field split on its `Group: Value` prefix into a namespace and a value (all 72 namespaces, including `Theme:`) | — (Dataset 1 has no equivalent field; the ~428 Dataset-1-only games simply have no Family tags) |
...
The relational schema is defined by Alembic migrations in `backend/alembic/versions/`, applied in order:
...
4. `00e27df745ad` — normalize categories/designers/mechanics/publishers into entity + join tables; drop the flat string from step 3
5. `f998ed3df8b5` — add `artists` + join table, indexes, `NOT NULL` constraints
...
11. `f52e87b1f96c` — add `themes` + join table
12. `de5ab409f353` — add ABSA tables: `review_aspects`, `game_aspect_aggregates`
...
15. `e7a2c58f9b16` — split the taxonomy correctly: `subdomains` (the old `categories` — BGG's rank/leaderboard type), `categories` (BGG's real Category field), `themes` (BGG Family's `Theme:` group)
...
17. `10441a9862cc` — add `families` + `game_families`, a first pass at the full BGG Family field modeled as one flat entity table (group name and value as plain columns on the same row)
18. `629567b8e65f` — replace step 17's flat design with a proper two-level hierarchy: `families` (the 72 namespaces, first-class rows) → `subfamilies` (the ~4,200 values, FK'd to their namespace) → `game_subfamilies` (games link to the leaf value only, since a game is never tagged with a bare namespace in the source data)
...
22. `b8e1c4a29f37` — add `game_embeddings` (one row per `(game_id, model)`, unsized `VECTOR`, unique on `(game_id, model)`); drop `games.embedding`, `embedding_model`, `embedding_updated_at` — a fixed-dim single column can't hold more than one embedding model's vectors at once, which blocks comparing/switching models
23. `c4d8f21a9e56` — `CREATE EXTENSION unaccent`; add a custom `english_unaccent` text search config (copies `english`, runs `unaccent` before stemming) — plain `english` doesn't fold diacritics, so a query for a non-ASCII designer/artist/publisher name (~9-10% of them) matched zero rows unless the exact accented spelling was typed
24. `d91a4c7e3f28` — add `reviews.quality_score`/`is_absa_eligible` — replaces the old JSON-cache ABSA sampling scheme with persisted per-review eligibility, computed once over the full ~4.2M-review corpus by `scripts/filter_eligible_reviews.py` rather than a cache file capped at a pre-restricted sample
25. `129f9cdc157b` — add `review_aspects.prob_positive`/`prob_neutral`/`prob_negative` — the full 3-class softmax, not just the winning class's confidence, so `scripts/absa_extract_hf.py` can store every prediction (not only ones clearing a confidence bar) and defer any threshold decision to query time
26. `44cec28c864a` — add `reviews.absa_processed_at` (+ a partial index matching the resumable-run query shape) — true per-review resumability for `scripts/absa_extract_hf.py`, tracking "was this review attempted" independent of whether it produced any storable `review_aspects` rows; backfilled for reviews already represented in `review_aspects`
...
| `families` → `subfamilies` → `game_subfamilies` | The full BGG Family field as a two-level hierarchy: 72 namespace rows, ~4,200 value rows FK'd to their namespace, games linked to the leaf value. Exposed on `Game.families` (the ORM relationship name stays flat for API consistency with the other tag fields, even though the DB layer underneath it is two tables) |
| `game_relations` | Expansion/implementation/integration links between games, resolved by name (see [Master dataset construction](#master-dataset-construction)) |
| `users`, `ratings` | Numeric-only interactions (used for collaborative filtering) |
| `reviews` | Ratings with review text + detected `language`/`language_confidence` (fastText's top-1 probability; low-confidence guesses are kept, not discarded — see `scripts/detect_languages.py`) + `quality_score`/`is_absa_eligible` (persisted ABSA eligibility — see `scripts/filter_eligible_reviews.py`) + `absa_processed_at` (per-review resumability marker for `scripts/absa_extract_hf.py`, set regardless of whether the review yielded any storable aspect) |
| `review_aspects` | Per-review, per-aspect ABSA extraction — every winning prediction (17-aspect taxonomy × positive/negative/neutral sentiment), the full 3-class softmax (`prob_positive`/`prob_neutral`/`prob_negative`), plus confidence and an evidence sentence; `review_id` links back to `reviews` |
| `game_aspect_aggregates` | Per-game, per-aspect rollup of `review_aspects` (positive/negative/mixed/neutral counts, mean sentiment), restricted to positive/negative rows clearing `ABSAConfig.WINNER_PROB_THRESHOLD` — `mixed_count`/`neutral_count` are currently always 0, not yet wired into the rollup |
...
No `CHECK` constraints exist anywhere in the schema (e.g. no DB-level rating-range enforcement) — value validity is enforced only in application code, not the database.
...
- **Review quality/language scoring**: `backend/app/core/review_quality.py` (language gate reusing precomputed `reviews.language`/`language_confidence` → hard filters including a VADER zero-sentiment check → exact + SimHash near-dup removal → a weighted density/diversity/specificity/boilerplate score), applied at full-corpus scale by `scripts/filter_eligible_reviews.py`. Reviews scoring `≥ ABSAConfig.QUALITY_SCORE_THRESHOLD` (0.6, calibrated against a real 50K-review score distribution) are ABSA-eligible. Replaces the old `compute_quality_score()` heuristic (fastText-per-call, a hand-picked keyword list, a spam-repetition penalty) that used to be duplicated near-identically in `absa_filter.py` and `absa_extract_hf.py` — `absa_filter.py` (an earlier, frozen pilot path) still has its own inlined copy, decoupled from `ABSAConfig`, but the canonical path no longer does.
...
| **Master dataset** | The merged, cleaned output in `data/processed/`, built by `build_master_dataset.py` + `build_interactions_dataset.py` |
...
| **Aspect** | One of the 22 ABSA taxonomy labels (Gameplay, Rulebook, Downtime, etc.) |
