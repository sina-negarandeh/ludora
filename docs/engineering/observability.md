...
- **Logs**: `logging_config.py`'s structlog pipeline tees every event to a dedicated, non-propagating stdlib logger (`ludora.otel_export`) that an OTel `LoggingHandler` forwards as an OTLP log record, in addition to (not instead of) the normal console render. The same processor also injects the current span's `trace_id`/`span_id` into the event dict; the shipped copy is rendered via `structlog.processors.KeyValueRenderer` (quoted values, e.g. `trace_id='abc123...'`), a different shape than the console's own `ConsoleRenderer` output.

**The collector and backends** (`observability/`, opt-in via `make observability-up`, see `observability/README.md`): one OTel Collector receives all three signals and fans them out -- traces to Tempo, logs to Loki (native OTLP ingestion), metrics to its own `:8889` for Prometheus to scrape. Grafana's three datasources are auto-provisioned with trace↔log↔metric correlation wired in (`Tempo.tracesToLogsV2` keyed on the quoted `trace_id='...'` text the Loki log line contains).

Degrades the same way the local LLM server does (`backend/AGENTS.md`): if the collector isn't running, OTLP exports fail/retry in the background and every endpoint keeps working exactly as before. Nothing here is required to run the app. `configure_otel()` is idempotent (a module-level guard), safe to call more than once in the same process.

**Langfuse** (`langfuse/`, opt-in via `make langfuse-up`, see `langfuse/README.md`): `AssistantService` imports `from langfuse.openai import OpenAI` instead of the plain SDK, tracing every `parse_query()`/`parse_plan()` call with the full prompt, completion, token usage, latency, and model params -- content the generic OTel setup above never captures. Composes with, doesn't fight, the OTel setup above: Langfuse's SDK reuses an already-registered `TracerProvider` rather than replacing it, and its own span processor only ever exports spans it recognizes as its own, so LLM traces go to Langfuse and HTTP/DB traces go to Tempo, never crossed. `AssistantService.__init__` calls `configure_otel()`/`configure_langfuse()` defensively (both idempotent), not just relying on `app.main` having done it first -- closes a real race where Langfuse's SDK claims the global `TracerProvider` for itself if none is registered yet, which would silently drop every FastAPI/SQLAlchemy/httpx span. `langfuse_config.py` constructs the shared `Langfuse` client explicitly from `Settings` (`public_key`/`secret_key`/`host`), not via `os.environ` mutation. Verified with a real call, same `trace_id` correlation as everything else in this doc.
...
- `GET http://localhost:3100/loki/api/v1/query_range?query={service_name="ludora-backend"}` returned log lines carrying the identical `trace_id`/`span_id` as the corresponding Tempo trace, rendered as `method='GET' path='/api/games/' status_code=200 duration_ms=151.9 ... trace_id='01ce9c...' span_id='cd94c5...'`, confirming the Grafana Loki datasource's `derivedFields` regex (`trace_id='(\w+)'`) actually matches real log lines, not just the config's intent.
