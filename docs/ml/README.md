...
- Recommendation and CF evaluation scripts can log to MLflow and write a results file, but haven't been run to produce a committed one yet; only search evaluation (MRR/NDCG/Recall) has ([evaluation.md](evaluation.md)).
- ABSA classification is running in resumable chunks and has attempted 39,484 of 267,950 eligible reviews so far (about 14.7%), not the full corpus yet ([absa.md](absa.md#coverage-full-corpus-filtered-not-sampled)).
