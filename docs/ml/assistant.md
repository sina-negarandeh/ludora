...
Recovery drops only the numeric range bounds (`min/max_complexity`, `min/max_playtime`, `min/max_year`) and never taxonomy filters or player counts. That split is not arbitrary. Rules 14 and 16 above instruct the model to *invent* those numbers when the user said something vague ("light" becomes `max_complexity=2.0`, "quick" becomes `max_playtime=30`), so they are the model's guess at a soft preference rather than something the user stated. Subdomains, categories and player counts are what was actually asked for; loosening those would answer a different question.

The branch only fires when the step matched nothing **and** a later step needs its result (otherwise "no matches" is itself the complete answer) **and** that step has not already been relaxed **and** there is a model-invented bound left to drop. When it does fire, the response says so: *"Nothing matched every constraint, so I relaxed min_complexity. Based on Blood on the Clocktower: ..."* Silently answering a looser question than the one asked would be worse than failing.
...
Because the cycle is the one place unbounded looping is even possible, it is the part covered by tests: `backend/tests/test_plan_executor.py` drives the graph with a fake orchestrator (no database, no LLM server, so it runs in CI) and pins that `relax` fires at most once per step, never fires when nothing depends on the empty step, and preserves taxonomy filters while dropping invented bounds.

### Resolving a placeholder: three shapes

`resolve_step()` handles every dependency a step can have uniformly:
