...
- MLflow experiment: `recommender/graph` (run name `graph_jaccard_precompute`), logging the seven relation weights, `n_games`, `recs_per_model_limit`, and `recommendations_written`. Same experiment as `deepwalk` so the two graph-based approaches are directly comparable as runs.
...
- Full catalog coverage confirmed live: 28,205 of 28,208 games have precomputed rows.
